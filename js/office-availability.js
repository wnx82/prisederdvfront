...
function ModalErreur(text) {
  // document.getElementById("ModalSuppLabel").innerHTML = titre;

  $("#textModalErreur").html(text);
...
officeAvailability.init = async function () {
  ...
  officeAvailability.availability = await $.get(
    `${app.api}/office-availability/${app.secondCurrentId}`
  );
...
officeAvailability.save = async (event) => {
  ...
  if (
    ...
    ModalErreur("Tous les champs sont obligatoires.");
  ...
  try {
    ...
    if (e.responseJSON && e.responseJSON.error) {
      ModalErreur(e.responseJSON.error);
